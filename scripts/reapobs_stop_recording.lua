...
local function get_rec_start_position()
  -- Try ExtState (set by start_recording scripts)
  ...
  -- Fallback: find the last REC START marker
...
-- last few minutes (to avoid picking up old files).
-- ------------------------------------------------------------
...
local function convert_video(input_path, output_dir)
  ...
  if input_ext and input_ext:lower() == VIDEO_FORMAT:lower() then
    -- If source and destination are the same file, nothing to do
...
-- Returns the bus track. Creates it at position 0 (topmost)
-- if it does not exist. Does not touch any other tracks.
-- ------------------------------------------------------------
...
-- Returns the newly created track. Properly adjusts folder
-- depth values so that existing tracks are not affected.
-- ------------------------------------------------------------
local function insert_child_in_folder(bus_idx, track_name)
  local count = reaper.CountTracks(0)

  -- Walk the folder to find the last child track
  local depth = 0
  local last_child_idx = bus_idx -- fallback: insert right after bus
  ...
  for i = bus_idx + 1, count - 1 do
    ...
    depth = depth + fd
    if depth < 0 then
      -- Track i has the folder-closing depth; it is the last child
      last_child_idx = i
  ...
  -- If the folder has no children yet (depth never went negative),
  -- the new track goes right after the bus
  local insert_idx
...
local function auto_import_video()
  ...
  -- Prevent undo from recording every sub-step
  reaper.Undo_BeginBlock()

  -- Create track structure: Videos bus → child track
