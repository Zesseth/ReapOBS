...
-- Marker name prefix for start markers (used to find alignment position)
...
-- Tries ExtState first (set by start scripts), then falls back
-- to scanning for the most recent REC START marker.
-- ------------------------------------------------------------
...
-- Scans for common video file extensions modified within the
-- last 5 minutes (to avoid picking up old files).
-- ------------------------------------------------------------
...
-- Returns the bus track, its index, and whether it was newly
-- created. Does not touch any other tracks.
-- ------------------------------------------------------------
local function get_or_create_videos_bus()
  ...
  -- Create at index 0 (topmost, below master)
...
-- ------------------------------------------------------------
-- Helper: insert a new child track at the end of an existing
-- folder. Only call this when the folder already has a closing
-- track (I_FOLDERDEPTH < 0). Adjusts the closing track's depth
-- to accommodate the new child.
-- ------------------------------------------------------------
local function insert_child_in_folder(bus_idx, track_name)
  ...
  for i = bus_idx + 1, count - 1 do
    ...
    if depth < 0 then
      last_child_idx = i
      -- Remove one level of folder closing from this track
...
-- Called after both REAPER and OBS have stopped recording.
-- ------------------------------------------------------------
local function auto_import_video()
  ...
  -- Get REAPER project path for the output location
  ...
  -- Convert (or remux) the video to the project directory
  ...
  -- Get the recording start position for alignment
  local rec_start = get_rec_start_position()

  -- Wrap track/item creation in an undo block
  reaper.Undo_BeginBlock()

  local video_name = filename_no_ext(converted_path)
  local bus, bus_idx, is_new_bus = get_or_create_videos_bus()
  local child_track

  if is_new_bus then
    -- Bus was just created — insert child directly after it so the folder
    -- is immediately self-contained and existing tracks are never swallowed
    reaper.InsertTrackAtIndex(bus_idx + 1, true)
    child_track = reaper.GetTrack(0, bus_idx + 1)
    reaper.GetSetMediaTrackInfo_String(child_track, "P_NAME", video_name, true)
    reaper.SetMediaTrackInfo_Value(child_track, "I_FOLDERDEPTH", -1)
    log("Created child track '" .. video_name .. "' at index " .. tostring(bus_idx + 1))
  else
    -- Bus already exists with proper closing structure — safe to walk and append
    child_track = insert_child_in_folder(bus_idx, video_name)
  end

  -- Import the video file onto the child track
